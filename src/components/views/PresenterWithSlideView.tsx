  ...
  return (
    <div className="w-full h-full relative p-8 flex items-center justify-center bg-transparent font-sans overflow-hidden">
      <div className="w-full h-full flex items-stretch justify-center gap-6 overflow-hidden">
        ...
        {/* RIGHT COLUMN: Presenter Camera Box (Stretches cleanly to match Slide Height) */}
        <motion.div
          initial={{ opacity: 0, scale: 0.96, x: 20 }}
          animate={{ opacity: 1, scale: 1, x: 0 }}
          transition={{ duration: 0.5, ease: 'easeOut', delay: 0.1 }}
          className={`flex-1 h-full max-h-[880px] min-w-[360px] bg-slate-950 ${shapeClass} overflow-hidden border-2 border-slate-700/80 shadow-[0_15px_40px_rgba(15,23,42,0.4)] relative shrink-0 self-stretch`}
        >
          <CameraBox
