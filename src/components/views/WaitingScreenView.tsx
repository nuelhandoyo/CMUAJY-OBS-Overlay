...
export const WaitingScreenView: React.FC<WaitingScreenViewProps> = ({ config }) => {
  ...
  const bgColor = config.waitingBgColor || config.customPrimaryBgColor || (config.themePreset === 'cream' ? '#FFF7E5' : '#093A6E');
  const accentColor = config.waitingAccentColor || config.customAccentColor || (config.themePreset === 'cream' ? '#093A6E' : '#A88337');

  const isLightBg = bgColor.toUpperCase().includes('FFF7E5') || bgColor.toUpperCase().includes('FAF3E0') || bgColor.toUpperCase().includes('FFFFFF') || bgColor.toUpperCase().includes('F8FAFC');
  ...
  return (
    <div className={`w-full h-full relative flex flex-col items-center justify-center p-6 md:p-8 bg-transparent text-slate-900 overflow-hidden ${fontClass}`}>
      ...
      <motion.div
        ...
        className={`max-w-4xl w-full backdrop-blur-xl border-4 ${shapeClass} p-8 md:p-10 shadow-[0_25px_60px_rgba(15,23,42,0.4)] text-center flex flex-col items-center z-10 relative overflow-hidden`}
        style={{
      ...
      >
        {/* Status Badge */}
        <div className={`inline-flex items-center gap-2 font-black text-xs px-4 py-1.5 rounded-full uppercase tracking-widest mb-6 shadow-md ${
          isLightBg ? 'bg-[#093A6E] text-[#FFF7E5]' : 'bg-[#FFF7E5] text-[#093A6E]'
        }`}>
        ...
        <h1 className={`text-3xl md:text-5xl font-black tracking-tight leading-tight max-w-3xl ${
          isLightBg ? 'text-[#093A6E]' : 'text-[#FFF7E5]'
        }`}>
        ...
        {config.waitingSubtitle && (
          <p className={`text-base md:text-xl font-semibold mt-3 max-w-2xl ${
            isLightBg ? 'text-[#926C35]' : 'text-[#A88337]'
          }`}>
            {config.waitingSubtitle}
          </p>
        )}
        ...
        <div className="my-6 md:my-8 flex items-center justify-center gap-4">
          <div className={`p-4 min-w-[110px] shadow-2xl rounded-2xl border-2 ${
            isLightBg ? 'bg-white border-[#093A6E]' : 'bg-[#093A6E] border-[#FFF7E5]'
          }`}>
        ...
        {config.waitingMessage && (
          <div className={`text-xs md:text-sm font-medium mb-6 flex items-center gap-2 px-4 py-2 rounded-xl border ${
            isLightBg ? 'bg-amber-100/80 border-amber-300 text-[#093A6E]' : 'bg-[#093A6E]/80 border-[#FFF7E5]/40 text-[#FFF7E5]'
          }`}>
        ...
        {config.speaker.name && (
          <div className={`rounded-2xl p-3.5 max-w-md w-full flex items-center justify-center text-center shadow-lg border ${
            isLightBg ? 'bg-white/90 border-[#093A6E]/30 text-[#093A6E]' : 'bg-[#093A6E] border-[#FFF7E5]/40 text-[#FFF7E5]'
          }`}>
            <div>
